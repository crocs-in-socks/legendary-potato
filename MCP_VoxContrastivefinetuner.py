...
device = 'cuda:1'
encoder_type = 'VoxContrastivefinetuned_MCP_moreDataRandomNegative_ResNet18Encoder'
projection_type = 'VoxContrastivefinetuned_MCP_moreDataRandomNegative_ResNet18ProjectionHead'
classifier_type = 'VoxContrastivefinetuned_MCP_moreDataRandomNegative_ResNet18ClassificationHead'
date = '27_11_2023'

pretrained_model_path = '/mnt/fd67a3c7-ac13-4329-bdbb-bdad39a33bf1/LabData/models_retrained/experiments/Nov24/'
finetuned_model_path = '/mnt/fd67a3c7-ac13-4329-bdbb-bdad39a33bf1/LabData/models_retrained/experiments/Nov27/'
...
ResNet_encoder.load_state_dict(torch.load(f'{pretrained_model_path}MCP_moreData_ResNet_Encoder_24_11_2023_state_dict101.pth'))
classification_head.load_state_dict(torch.load(f'{pretrained_model_path}MP_moreData_ResNet_ClassificationHead_24_11_2023_state_dict101.pth'))
...
encoder_optimizer = optim.Adam([*ResNet_encoder.parameters(), *projection_head.parameters()], lr = 0.001, eps = 0.0001)
...
for epoch in range(1, number_of_epochs+1):
    ...
    for data in tqdm(ResNet_trainloader):
        ...
        oneHot_labels = data['lesion_labels'].float()
        ...
        out_list = [layer1[:current_batch_size], layer2[:current_batch_size], layer3[:current_batch_size], z_mixed[:current_batch_size]]
        projection_output = projection_head.forward(out_list)

        projection_output = F.interpolate(projection_output, size=(128, 128, 128))

        if not os.path.exists(f'./temporary/epoch#{epoch}_train'):
            os.makedirs(f'./temporary/epoch#{epoch}_train')

        plt.figure(figsize=(20, 15))
        plt.subplot(1, 4, 1)
        plt.imshow(projection_output[0, 0, :, :, 64].detach().cpu(), cmap='gray')
        ...
        plt.colorbar()
        plt.savefig(f'./temporary/epoch#{epoch}_train/sample#{idx}')
        plt.close()
        ...
        encoder_loss = encoder_criterion(projection_output, gt, subtracted)
    ...
    for data in tqdm(ResNet_validationloader):
        with torch.no_grad():
            ...
            if not os.path.exists(f'./temporary/epoch#{epoch}_val'):
                os.makedirs(f'./temporary/epoch#{epoch}_val')
            ...
            plt.imshow(subtracted[0, 0, :, :, 64].detach().cpu(), cmap='gray')
            plt.title(f'Epoch #{epoch} Subtracted GT #{idx+1}')
            plt.savefig(f'./temporary/epoch#{epoch}_val/sample#{idx}')
            plt.close()
