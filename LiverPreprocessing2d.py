...
c = Constants(
    ...
    num_epochs = 100,
    date = '24_01_2024',
    to_save_folder = 'Jan24',
    to_load_folder = None,
    device = 'cuda:1',
    proxy_type = 'LiTS_Unet2D_preprocessing_0_>_200_window_init_features_64_with_crop_then_median_filtering_and_AHE_cl_0_01_then_bilateral',
    ...
    dataset = '2dpreprocessed_lits'
)

trainset, validationset, testset = load_dataset(c.dataset, c.drive, ToTensor3D(labeled=True))

trainset = SliceDataset(trainset)
validationset = SliceDataset(validationset)
...
model = UNet2D(out_channels=2, init_features=64).to(c.device)
criterion = DiceLoss().to(c.device)
optimizer = optim.Adam(model.parameters(), lr = 0.001, eps = 0.0001)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', patience=2, factor=0.5, min_lr=0.0001, verbose=True)
...
for epoch in range(1, c.num_epochs+1):
    ...
    for data in tqdm(trainloader):
        ...
        # plt.subplot(1, 3, 2)
        # plt.imshow(segmentation[0, 1, :, :].detach().cpu())
        # plt.subplot(1, 3, 3)
        # plt.imshow(gt[0, 1, :, :].detach().cpu())
        # plt.savefig('./temp')
        # plt.close()
