...
ResNet_Model = ResNet3D_Encoder(image_channels=number_of_features).to(device)
...
classification_head.load_state_dict(torch.load(f'{ResNet_model_path}ResNet3D_classifier_patchlvl_32_state_dict145.pth'))
...
def get_layer_TSNE(layer_name, folder_name, perplexity):
    ...
    for (X, oneHot_X_labels) in tqdm(ResNet_testloader):
    ...
    for t, l in zip(leg.texts, new_labels):
        t.set_text(l)
    plt.title('ResNet Encoder Patchlvl Supervised Contrastive')
    plt.savefig(f'./{folder_name}/{layer_name}_p{perplexity}')
    plt.close()

get_layer_TSNE('layer3', 'temporary', 30)
